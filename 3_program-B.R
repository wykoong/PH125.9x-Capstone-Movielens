...
# thus by using movie_lambda=1.5, the RMSE is
predicted_movie_rating <- edx_test %>%
  left_join(movie_sums, by='movieId') %>%
  mutate(movie_effect = movie_rating_sum/(movie_rating_count+movie_lambda)) %>%
  mutate(predicted = mu + movie_effect) %>%
  pull(predicted)

movie_rmse = RMSE(predicted_movie_rating, edx_test$rating)
...
movie_sums <- edx_train %>%
  group_by(movieId) %>%
  summarise(movie_effect = sum(rating-mu) / (n()+user_movie_lambda)) %>%
  select(movieId,movie_effect)

user_sums <- edx_train %>%
  left_join(movie_sums, by="movieId") %>%
  group_by(userId) %>%
  summarise(user_effect = sum(rating-mu-movie_effect) / (n()+user_movie_lambda))
...
user_movie_rating_rmse <- RMSE(predicted_user_movie_rating, edx_test$rating)
